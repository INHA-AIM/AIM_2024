...
{
    TESTDRIVE1_FindNearIdx(pst_PlanningData);
    TESTDRIVE1_FindBankFlag(pst_PlanningData);
    TESTDRIVE1_FlagProcessing(pst_PlanningData, pst_CANData);    // Go, Stop, Slow on, Slow off, Pit Stop에 대한 Flag Planning 수행
    TESTDRIVE1_RestartZoneCheck(pst_PlanningData);               // PitStop에서 나갈 때, 차량이 진입하는지?

    TESTDRIVE1_EgoVehicleDataProcessing(pst_PlanningData, pst_DeadReckoningData, pst_CANData, pst_LidarData);
    TESTDRIVE1_ClusteredDataProcessing(pst_PlanningData, pst_DeadReckoningData, pst_LidarData, pst_CANData);

    TESTDRIVE1_FrenetPathGeneration(pst_PlanningData);            // 총 9개의 Frenet Path 생성
    TESTDRIVE1_CalcFrenetToGlobalPath(pst_PlanningData);          // Frenet Path 를 Global ENU 좌표로 변환

    TESTDRIVE1_FindAvailableFrenetLane(pst_PlanningData);               // 충돌하지 않는 프레넷 경로 상 여유라인을 찾아주는 함수
    TESTDRIVE1_FindAvailableGridLane(pst_PlanningData);

    TESTDRIVE1_VelocityPlanning(pst_PlanningData);
    TESTDRIVE1_StateProcessing(pst_LidarData, pst_DeadReckoningData, pst_CameraData, pst_PlanningData, pst_CANData);
    TESTDRIVE1_FinalPath(pst_PlanningData);                       // Final Path는 Control에 넘겨줘야 하는 최종 Path
}
...
void TESTDRIVE1_EgoVehicleDataProcessing(PLANNING_DATA_t *pst_PlanningData, DEAD_RECKONING_DATA_t *pst_DeadReckoningData, CAN_DATA_t *pst_CANData, HMC_LIDAR_DATA_t *pst_LidarData)
{
    ...
    TESTDRIVE1_PitStopProcessing(pst_PlanningData, &pst_Spline2DParam, &pst_SplinePath);
...
void TESTDRIVE1_ControlPath(PLANNING_DATA_t *pst_PlanningData, float32_t *f32_PathGapList, int32_t *s32_CurrentLaneValues)
{
    FRENET_PARAM_t *pst_FrenetParam = &pst_PlanningData->st_FrenetParam;
...
void TESTDRIVE1_FrenetPathGeneration(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    TESTDRIVE1_FrenetPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
    TESTDRIVE1_CollisionCheckPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
    TESTDRIVE1_ControlPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
}
...
void TESTDRIVE1_ProcessControlPath(SPLINE_PATH_t *pst_SplinePath, SPLINE2D_PARAM_t *pst_Spline2DParam, FRENET_PATH_t *pst_FrenetPath, int32_t s32_FrenetPathNum)
{
    int32_t s32_I = 0;
    int32_t s32_J = 0;
    float32_t f32_X, f32_Y, f32_Yaw_rad, f32_D0, f32_DX, f32_DY, f32_DS;
...
void TESTDRIVE1_CalcFrenetToGlobalPath(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    TESTDRIVE1_PitStopProcessing(pst_PlanningData, &pst_Spline2DParam, &pst_SplinePath);
    TESTDRIVE1_ProcessFrenetPath(pst_SplinePath, pst_Spline2DParam, pst_FrenetPath, pst_PlanningData->s32_FrenetPathNum);
    TESTDRIVE1_ProcessCollisionCheckPath(pst_SplinePath, pst_Spline2DParam, pst_CollisionCheckPath, pst_FrenetParam->s32_PathNum);
    TESTDRIVE1_ProcessControlPath(pst_SplinePath, pst_Spline2DParam, pst_ControlPath, pst_FrenetParam->s32_PathNum);
}
...
{
    PLANNER_t *pst_Planner = &pst_PlanningData->st_Planner;
    TESTDRIVE1_StraightZonePlanning(pst_PlanningData);
}
...
void TESTDRIVE1_StraightZonePlanning(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    bool b_InvadeLane = TESTDRIVE1_IsAccAvailable(pst_PlanningData); // 현재 차선에 차량 침범 여부 판단
    TESTDRIVE1_TargetLaneProcessing(pst_PlanningData);
    ...
    {
        case STATE_IDLE:
            pst_PlanningData->s32_BestPath = TESTDRIVE1_SelectFrenetBestPath(pst_PlanningData, 9);
            break;
...
void TESTDRIVE1_AccProcessing(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    int32_t s32_TargetAccIdx = TESTDRIVE1_SelectTargetAccIdx(pst_PlanningData);
    ...
    else
    {
        ...
        pst_PlanningData->f32_TargetSpeed_kph = pst_ObjectData[s32_TargetAccIdx].f32_Velocity_m_s_global * 3.6f + 10.f;
    }
...
void TESTDRIVE1_VelocityPlanning(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    if (!pst_Planner->s32_LabCount 
        && 200 < pst_PlanningData->st_ReferenceSELine2_global.s32_NearIdx
        && pst_PlanningData->st_ReferenceSELine2_global.s32_NearIdx < 501)
    {
        pst_PlanningData->s32_SpeedSignal = 1;
        pst_PlanningData->f32_TargetSpeed_kph = 30.f;
    }
    else
    {
        pst_PlanningData->s32_SpeedSignal = 0;
...
void TESTDRIVE1_FinalPath(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    if (!pst_Planner->s32_LabCount
        && 200 < pst_PlanningData->st_ReferenceSELine2_global.s32_NearIdx
        && pst_PlanningData->st_ReferenceSELine2_global.s32_NearIdx < 551
        && st_VehicleState != STATE_OVERTAKING)
    {
        pst_PlanningData->s32_BestPath = TESTDRIVE1_SelectFrenetBestPath(pst_PlanningData, 17);
    }

    int32_t s32_MaxLaneChange = 9;
    int32_t s32_NearIdx = 0;

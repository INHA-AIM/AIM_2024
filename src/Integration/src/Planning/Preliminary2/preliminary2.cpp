...
{
    PRE2_FindNearIdx(pst_PlanningData);
    PRE2_FindBankFlag(pst_PlanningData);
    PRE2_FlagProcessing(pst_PlanningData, pst_CANData);

    PRE2_EgoVehicleDataProcessing(pst_PlanningData, pst_DeadReckoningData, pst_CANData, pst_LidarData);
    PRE2_ClusteredDataProcessing(pst_PlanningData, pst_DeadReckoningData, pst_LidarData, pst_CANData);

    PRE2_FrenetPathGeneration(pst_PlanningData);
    PRE2_CalcFrenetToGlobalPath(pst_PlanningData);

    PRE2_FindAvailableFrenetLane(pst_PlanningData);
    PRE2_FindAvailableGridLane(pst_PlanningData);

    PRE2_StateProcessing(pst_LidarData, pst_DeadReckoningData, pst_CameraData, pst_PlanningData, pst_CANData);
    PRE2_VelocityPlanning(pst_PlanningData);
    PRE2_FinalPath(pst_PlanningData);
}
...
void PRE2_EgoVehicleDataProcessing(PLANNING_DATA_t *pst_PlanningData, DEAD_RECKONING_DATA_t *pst_DeadReckoningData, CAN_DATA_t *pst_CANData, HMC_LIDAR_DATA_t *pst_LidarData)
{
    ...
    PRE2_PitStopProcessing(pst_PlanningData, &pst_Spline2DParam, &pst_SplinePath);
...
void PRE2_FrenetPathGeneration(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    PRE2_FrenetPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
    PRE2_CollisionCheckPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
    PRE2_ControlPath(pst_PlanningData, f32_PathGapList, s32_CurrentLaneValues);
}
...
void PRE2_CalcFrenetToGlobalPath(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    PRE2_PitStopProcessing(pst_PlanningData, &pst_Spline2DParam, &pst_SplinePath);
    PRE2_ProcessFrenetPath(pst_SplinePath, pst_Spline2DParam, pst_FrenetPath, pst_PlanningData->s32_FrenetPathNum);
    PRE2_ProcessCollisionCheckPath(pst_SplinePath, pst_Spline2DParam, pst_CollisionCheckPath, pst_FrenetParam->s32_PathNum);
    PRE2_ProcessControlPath(pst_SplinePath, pst_Spline2DParam, pst_ControlPath, pst_FrenetParam->s32_PathNum);
}
                                  ...
                                  CAN_DATA_t *pst_CANData)
{
    ...
    if (b_PrevBank != pst_Planner->b_PlanningBank)
    {
        ...
        PRE2_InitAccMode(pst_PlanningData, pst_CANData);
    }

    switch(pst_Planner->b_PlanningBank)
    {
        case false:  // No Bank
            PRE2_StraightZonePlanning(pst_PlanningData, pst_CANData);
            break;

        case true: // Bank
            PRE2_BankZonePlanning(pst_PlanningData, pst_CANData);
            break;
    }
    ...
    {
        pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, s32_StartLane);
    }
}
...
void PRE2_StraightZonePlanning(PLANNING_DATA_t *pst_PlanningData, CAN_DATA_t *pst_CANData)
{
    ...
    bool b_InvadeLane = PRE2_IsAccAvailable(pst_PlanningData);
    bool b_SafeToOvertaking = PRE2_IsOverTakingAvailable(pst_PlanningData);
    ...
    {
        case STATE_IDLE:
            pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, 1);
            if (b_InvadeLane)
            {
                st_VehicleState = STATE_IDLE_ACC_MODE;
            }

            if (PRE2_IsLeftObject(pst_PlanningData, pst_EgoVehicleData->s32_CurrentLane))
            {
                pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, pst_EgoVehicleData->s32_CurrentLane);
            }
        ...
        case STATE_IDLE_ACC_MODE:
            ...
            if (b_InvadeLane)
            {
                PRE2_AccProcessing(pst_PlanningData);
                pst_Planner->b_AccMode = true;

                if (b_SafeToOvertaking && pst_Planner->s32_LabCount != 0)
                {
        ...
        case STATE_OVERTAKING:  // OverTaking Mode가 시작되었을 때
            PRE2_TargetLaneProcessing(pst_PlanningData);
            ...
            if (PRE2_IsOverTakingSuccess(pst_PlanningData))
            {
       ...
       case STATE_STATIC:
        ...
        case STATE_PITSTOP:
            pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, 17);
            printf("PitStop\n");
            if (PRE2_IsRightObject(pst_PlanningData, pst_EgoVehicleData->s32_CurrentLane))
            {
                pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, pst_EgoVehicleData->s32_CurrentLane);
            }
...
void PRE2_BankZonePlanning(PLANNING_DATA_t *pst_PlanningData, CAN_DATA_t *pst_CANData)
{
    ...
    bool b_InvadeLane = PRE2_IsAccAvailable(pst_PlanningData); // 현재 차선에 차량 침범 여부 판단
    bool b_SafeToOvertaking = PRE2_IsOverTakingAvailable(pst_PlanningData); // 추월 가능 여부 판단

    switch (st_VehicleState)
    {
        case STATE_IDLE: // Idle 상태는 기본 주행 상태
            pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, 1);
            if (PRE2_IsLeftObject(pst_PlanningData, pst_PlanningData->st_EgoVehicleData.s32_CurrentLane))
            {
                pst_PlanningData->s32_BestPath = PRE2_SelectFrenetBestPath(pst_PlanningData, pst_PlanningData->st_EgoVehicleData.s32_CurrentLane);
            }
            if (b_InvadeLane)
            {
                st_VehicleState = STATE_IDLE_ACC_MODE;
...
void PRE2_AccProcessing(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    int32_t s32_TargetAccIdx = PRE2_SelectTargetAccIdx(pst_PlanningData);
    ...
    pst_ObjectData[s32_TargetAccIdx].f32_TargetVelocity_kph = pst_ObjectData[s32_TargetAccIdx].f32_Velocity_kph_global + f32_Kp * (pst_ObjectData[s32_TargetAccIdx].f32_RelativeDistance - pst_ObjectData[s32_TargetAccIdx].f32_SafetyDistance);
    pst_ObjectData[s32_TargetAccIdx].f32_TargetVelocity_m_s = pst_ObjectData[s32_TargetAccIdx].f32_TargetVelocity_kph / 3.6f;
...
{
    bool b_Condition_1 = PRE2_IsAccTargetLow(pst_PlanningData);
    bool b_Condition_2 = PRE2_IsWidePlace(pst_PlanningData);
...
void PRE2_TargetLaneProcessing(PLANNING_DATA_t *pst_PlanningData)
{
    ...
    pst_PlanningData->s32_BestPath = PRE2_SelectOverTakingPath(pst_PlanningData);
